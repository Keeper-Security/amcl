...
def rsaset(tb,nb,base,ml) :
    ...
    run_in_shell("mkdir -p amcl"+slashtext+tb)
...
def curveset(tc,nb,base,nbt,m8,mt,ct,pf,stw,sx,ab,cs) :
    ...
    try:
        ...
        sys.stderr.write("Error in script.  Use AMCL source files in amcl/")

run_in_shell("echo $PWD")
run_in_shell("mkdir -p amcl")
